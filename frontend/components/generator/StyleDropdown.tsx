  ...
  return (
    <div ref={ref} className="relative space-y-3">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full cursor-pointer items-center justify-between rounded-[1rem] border border-[rgba(194,198,214,0.55)] bg-(--surface-container-low) px-4 py-3 text-left tonal-transition hover:bg-(--surface-container)"
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-(--on-surface)">{selected?.label}</p>
            <p className="text-xs text-(--outline)">{selected?.desc}</p>
          </div>
        </div>
        <ChevronDownIcon
          className={`size-5 text-(--outline) tonal-transition ${open ? 'rotate-180' : ''}`}
        />
      </button>

      {open && (
        <div className="absolute z-20 w-full rounded-[1.25rem] border border-[rgba(194,198,214,0.35)] bg-(--surface-container-lowest) p-2 shadow-[0_20px_40px_rgba(25,28,30,0.18)]">
          {styleOptions.map((option) => (
            <button
              ...
              className="flex w-full cursor-pointer items-start gap-3 rounded-[1rem] px-3 py-3 text-left tonal-transition hover:bg-(--surface-container-low)"
            >
              <span className="mt-0.5 text-(--primary)">
              ...
              <div>
                ...
                <p className="text-xs leading-relaxed text-(--outline)">{option.desc}</p>
              </div>
            </button>
          ))}
        </div>
