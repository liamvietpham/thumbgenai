  ...
  return (
    <section className="relative flex flex-col items-center overflow-hidden px-8 py-24 text-center md:py-32">
      <div className="pointer-events-none absolute left-1/2 top-0 h-full w-full -translate-x-1/2 -z-10 opacity-30">
        <div className="absolute left-[-10%] top-[-10%] h-[60%] w-[40%] rounded-full bg-(--secondary-container) blur-[120px]" />
        <div className="absolute bottom-[-10%] right-[-10%] h-[60%] w-[40%] rounded-full bg-(--primary-container) blur-[120px]" />
      </div>

      <div className="max-w-4xl space-y-6">
        <p className="inline-flex items-center gap-2 rounded-full bg-[rgba(87,223,254,0.28)] px-4 py-1.5 text-xs font-bold uppercase tracking-[0.2em] text-(--on-secondary-container)">
          <LightningIcon className="size-4" aria-hidden="true" />
          New: Thumbnail AI v2.0
        </p>

        <h1 className="font-headline text-5xl font-bold leading-[1.1] tracking-tight text-(--on-surface) md:text-7xl">
        ...
        <div className="flex flex-col items-center justify-center gap-4 pt-8 sm:flex-row">
          <Link
            href="/generate"
            className="inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-linear-to-br from-[#0058be] to-[#2170e4] px-10 py-4 text-lg font-bold text-white shadow-[0_20px_40px_-12px_rgba(0,88,190,0.3)] tonal-transition hover:-translate-y-0.5 active:scale-95 sm:w-auto"
          >
            Get Started
          ...
          <Link
            href="/how-it-works"
            className="inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-(--surface-container-low) px-10 py-4 text-lg font-semibold text-(--on-surface) tonal-transition hover:bg-(--surface-container) sm:w-auto"
          >
            <EyeIcon className="size-5 text-(--primary)" aria-hidden="true" />
            See How It Works
          </Link>
        </div>
      </div>
      ...
      <div className="relative mx-auto mt-20 w-full max-w-5xl">
        ...
        <aside className="glass-card absolute -bottom-6 -left-6 hidden max-w-[240px] rounded-2xl p-6 shadow-xl lg:block">
          <div className="mb-3 flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-full bg-(--tertiary-fixed) text-[#102000]">
            ...
            <div className="text-left">
              ...
              <p className="text-sm font-bold text-(--on-surface)">98% Click Depth</p>
            </div>
          </div>
        ...
        <aside className="glass-card absolute -right-6 -top-10 hidden rounded-2xl p-5 shadow-xl lg:block">
          <div className="flex items-center gap-4">
            <div className="space-y-1">
              <p className="text-xs font-bold tracking-[0.16em] text-(--secondary)">
                REALTIME
              </p>
              <p className="text-sm font-bold text-(--on-surface)">Generating Variant C…</p>
            </div>
