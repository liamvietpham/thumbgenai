...
export function FeaturesSection() {
  return (
    <section
      ...
      className="bg-[var(--surface-container-low)] px-6 py-24 md:px-8"
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-14 grid gap-8 md:grid-cols-3">
          {features.map(({ title, description, icon: Icon }) => (
            <article
              key={title}
              className="group rounded-[2rem] bg-[var(--surface-container-lowest)] p-8 shadow-sm ring-1 ring-[rgba(194,198,214,0.42)] tonal-transition hover:-translate-y-1 hover:shadow-[0_24px_48px_-28px_rgba(0,88,190,0.22)]"
            >
              <div className="grid size-16 place-items-center rounded-2xl bg-[var(--surface)] text-[var(--primary)] shadow-sm tonal-transition group-hover:scale-105 group-hover:shadow-md">
                <Icon className="size-8" aria-hidden="true" />
              </div>
              <h3 className="mt-6 font-headline text-2xl font-bold text-[var(--on-surface)]">
                {title}
              </h3>
