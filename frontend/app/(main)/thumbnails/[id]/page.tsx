  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-16 sm:px-6 md:pb-24 lg:px-8">
      ...
      <div className="mb-8 pt-8">
        <Link
          href="/thumbnails"
          className="inline-flex items-center gap-1.5 text-sm font-medium text-(--on-surface-variant) tonal-transition hover:text-(--primary)"
        >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="space-y-6 lg:col-span-8">
          {/* Thumbnail preview */}
          <div className="relative aspect-video overflow-hidden rounded-[1.75rem] bg-(--surface-container-highest) shadow-[0_8px_40px_rgba(0,88,190,0.10)]">
            <Image
              src={item.imageUrl}
              alt={item.title}
              fill
          ...
          <div className="flex flex-col gap-2.5 sm:flex-row sm:flex-wrap sm:gap-3">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-linear-to-br from-[#0058be] to-[#2170e4] px-5 py-3 text-sm font-semibold text-white shadow-md shadow-[rgba(0,88,190,0.20)] tonal-transition hover:scale-[1.02] sm:py-2.5"
            >
            ...
            <Link
              href="/regenerate"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-(--surface-container-high) px-5 py-3 text-sm font-semibold text-(--primary) tonal-transition hover:bg-(--surface-container-highest) sm:py-2.5"
            >
            ...
            <Link
              href="/generate"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-(--surface-container-high) px-5 py-3 text-sm font-semibold text-(--on-surface-variant) tonal-transition hover:bg-(--surface-container-highest) sm:py-2.5"
            >
              Generate New
          ...
          <div className="rounded-[1.75rem] border border-[rgba(194,198,214,0.5)] bg-(--surface-container-lowest) p-8 shadow-[0_4px_32px_rgba(0,88,190,0.04)]">
            ...
            <div className="mb-10 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="flex size-10 shrink-0 items-center justify-center rounded-full bg-(--primary-fixed)">
                  <svg aria-hidden="true" className="size-5 text-(--primary)" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} viewBox="0 0 24 24">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-[rgba(194,198,214,0.3)] bg-(--surface-container-low) px-4 py-3">
                <ChipIcon className="size-5 shrink-0 text-(--primary)" />
                <div>
                  <p className="text-sm font-bold text-(--on-surface)">Gemini Flash 2.0</p>
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-12">
              ...
              <div className="space-y-8 md:col-span-7">
                ...
                {/* Title or Topic */}
                <div>
                  <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-(--outline)">
                    Title or Topic
                  </p>
                  <p className="rounded-2xl border border-[rgba(194,198,214,0.2)] bg-(--surface-container-low) p-5 text-sm italic leading-relaxed text-(--on-surface-variant)">
                ...
                <div>
                  ...
                  {item.additionalPrompts ? (
                    <p className="rounded-2xl border border-[rgba(194,198,214,0.2)] bg-(--surface-container-low) p-5 text-sm leading-relaxed text-(--on-surface-variant)">
                      {item.additionalPrompts}
                    </p>
                  ) : (
                    <p className="rounded-2xl border border-[rgba(194,198,214,0.2)] bg-(--surface-container-low) p-5 text-sm italic text-(--outline)">
                ...
                {/* Reference Images */}
                <div>
                  <p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-(--outline)">
                    Reference Images
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {refImages.map((src, i) => (
                      <div
                        key={src}
                        className="relative aspect-square overflow-hidden rounded-xl border border-[rgba(194,198,214,0.4)]"
                      >
                        <Image
                          src={src}
                          alt={`Reference ${i + 1}`}
                    ...
                    {Array.from({ length: emptySlots }).map((_, i) => (
                      <div
                        key={`empty-${i}`}
                        className="flex aspect-square flex-col items-center justify-center gap-1 rounded-xl border-2 border-dashed border-[rgba(194,198,214,0.6)] bg-(--surface-container-low) text-(--on-surface-variant)/40"
                      >
              ...
              <div className="space-y-6 md:col-span-5">
                ...
                <div>
                  ...
                  {item.userPhoto ? (
                    ...
                    <div className="flex aspect-4/5 flex-col items-center justify-center gap-3 rounded-[1.25rem] border-2 border-dashed border-[rgba(194,198,214,0.6)] bg-(--surface-container-low) text-(--on-surface-variant)">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-[rgba(172,237,255,0.7)] bg-[rgba(172,237,255,0.2)] p-4 shadow-sm">
                    <PaletteIcon className="size-5 shrink-0 text-(--secondary)" />
                    <span className="text-sm font-bold text-(--secondary)">{item.style}</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-[rgba(194,198,214,0.4)] bg-(--surface-container-high) p-4 shadow-sm">
                    <SquareIcon className="size-5 shrink-0 text-(--on-surface-variant)" />
                    <span className="text-sm font-bold text-(--on-surface-variant)">{aspectLabel}</span>
                  </div>
                </div>
        ...
        <div className="space-y-6 lg:col-span-4">
          ...
          <div className="rounded-[1.75rem] border border-[rgba(194,198,214,0.5)] bg-(--surface-container-lowest) p-8 shadow-[0_4px_32px_rgba(0,88,190,0.04)]">
            ...
            {/* CTR Score */}
            <div className="mb-6 rounded-2xl bg-(--surface-container-low) p-5 text-center">
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-(--outline)">
                CTR Score
              </p>
              <p className="font-headline mt-1 text-5xl font-bold text-(--primary)">94%</p>
              <p className="mt-1 text-xs text-(--on-surface-variant)">Estimated click-through rate</p>
              <span className="mt-3 inline-block rounded-full bg-(--tertiary-fixed) px-3 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#102000]">
                High Potential
              </span>
            </div>
            ...
            <div className="border-t border-[rgba(194,198,214,0.3)] pt-6">
              <p className="mb-4 text-[11px] font-bold uppercase tracking-[0.2em] text-(--outline)">
                Color Palette
              </p>
          ...
          <div className="rounded-[1.75rem] bg-linear-to-br from-[#0058be] to-[#2170e4] p-8 text-white shadow-[0_4px_32px_rgba(0,88,190,0.18)]">
