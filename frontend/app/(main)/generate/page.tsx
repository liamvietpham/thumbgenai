            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-(--outline)">{title.length}/100</span>
                </div>
                <input
                  maxLength={100}
              ...
              <div className="space-y-2 border-t border-(--surface-container) pt-5">
                ...
                <div className="relative size-32 overflow-hidden rounded-[1rem]">
                  {/* Upload state */}
                  <label className={`absolute rounded-[1rem] inset-0 flex cursor-pointer flex-col items-center justify-center gap-2 border-2 border-dashed border-[rgba(194,198,214,0.7)] bg-(--surface-container-low) tonal-transition hover:bg-(--surface-container) ${userPhotoPreview ? 'pointer-events-none opacity-0' : 'opacity-100'}`}>
                    <div className="grid size-10 place-items-center rounded-xl bg-(--surface-container-highest) text-(--primary) shadow-sm">
                      <UploadIcon className="size-5" aria-hidden="true" />
                    </div>
                    <p className="text-sm font-medium text-(--on-surface)">Upload photo</p>
                    <p className="text-xs text-(--outline)">Face reference</p>
              ...
              <div className="space-y-2 border-t border-[rgba(194,198,214,0.7)] pt-5">
                ...
                <div className="flex gap-3">
                  {refImages.map((img, i) => (
                    <div key={i} className="group relative h-32 w-32 shrink-0 rounded-[1rem] bg-(--surface-container)">
                  ...
                  {refImages.length < 2 && (
                    <label className="flex h-32 w-32 shrink-0 cursor-pointer flex-col items-center justify-center gap-1 rounded-[1rem] border-2 border-dashed border-[rgba(194,198,214,0.7)] bg-(--surface-container-low) text-(--outline) tonal-transition hover:bg-(--surface-container)">
